...
func (m *RecordModel) InsertRecord(ctx context.Context, record *Record) (int, error) {
...
func (m *RecordModel) GetIdAndTitles(ctx context.Context, user_id int32) ([]IdAndTitle, error) {
	var record []IdAndTitle
	// sql := "select from Record where user_id=?"
	// result := m.db.Raw(sql, user_id).Scan(&record)
	result := m.db.WithContext(ctx).Where("user_id = ?", user_id).Select("id, title").Find(&record)
